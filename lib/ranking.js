...
router.get("/tracks/:year/:month", function(req, res) {
  aggregatedByMonth("tracks_by_month",
    req.params.year + "-" + req.params.month, res);
});

/**
 * @param month e.g. 2016-01
 */
function aggregatedByMonth(view, month, res) {
  ...
  db.view("ranking", view, {
    ...
    if (err) {
      ...
      summarizeAndSort(result.entries);
...
/**
 * Removes duplicate entries for the same artist, summing the count and keeping the duplicate entries as children
 */
function summarizeAndSort(entries) {
  // first sort entries on the artist
  entries.sort(function (song1, song2) {
